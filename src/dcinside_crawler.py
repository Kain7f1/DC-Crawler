    ...
    for search_pos in range(max_number, 0, -10000):
        ...
        for page in range(1, last_page+1):          # page = 1만 단위 검색결과 페이지
            ...
            for element in element_list:
                try:
                    new_row, is_ignore, is_black = cr.get_url_row(element, search_info, blacklist, whitelist)
            ...
            for reply in reply_list:
                try:
                    if cr.is_ignore_reply(reply):   # [무의미한 댓글 넘어가기]
                        continue
                    # [댓글 정보 가져오기]
                    new_row, is_ignore = cr.get_reply_row(url_row, reply)
                    if is_ignore:
                        continue
